...
class Logs
{
    ...
    public function write($logsAll)
    {
        ...
        foreach ($logsAll as $key => $value) {
            ...
            /*
             * 修改源代码,采用协程的方式实现，原先采用PHP原始的文件操作函数实现输出.
             * swoole在4.0.4版本以上实现了协程的非文件stream流操作，比直接使用原始文件操作函数有着更加好的性能
            */
            if (version_compare(\swoole_version(),"4.0.4",">")){
                $fp = @fopen( "php://stdout", "a");
                \Swoole\Coroutine::create(function ()use($fp,$value){
                    \Swoole\Coroutine::fwrite($fp,$value);
                });
            }else{
                //日志分类文件夹
